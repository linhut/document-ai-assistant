...
class FormatExtractor:
    ...
    def _generate_rules(self, sections: dict, ps: dict) -> tuple[list, list]:
        ...
        if title:
            if title.get("font"):
                ref = _add_check("title.font", title["font"], "标题字体检查", f"标题应使用{title['font']}")
                _add_fix(ref, "set_font", "doc_title", title["font"])
            if title.get("size"):
                ref = _add_check("title.size", title["size"], "标题字号检查", f"标题应使用{title['size']}")
                _add_fix(ref, "set_size", "doc_title", title["size"])
            if title.get("align"):
                ref = _add_check(
                    "title.align", title["align"], "标题对齐检查", f"标题应{self._align_label(title['align'])}"
                )
        ...
        if h1:
            if h1.get("font"):
                ref = _add_check("heading_1.font", h1["font"], "一级标题字体检查", f"一级标题应使用{h1['font']}")
                _add_fix(ref, "set_font", "heading_1", h1["font"])
            if h1.get("size"):
                ref = _add_check("heading_1.size", h1["size"], "一级标题字号检查", f"一级标题应使用{h1['size']}")
        ...
        if h2:
            if h2.get("font"):
                ref = _add_check("heading_2.font", h2["font"], "二级标题字体检查", f"二级标题应使用{h2['font']}")
                _add_fix(ref, "set_font", "heading_2", h2["font"])
            if h2.get("size"):
                ref = _add_check("heading_2.size", h2["size"], "二级标题字号检查", f"二级标题应使用{h2['size']}")
        ...
        if h3:
            if h3.get("font"):
                ref = _add_check("heading_3.font", h3["font"], "三级标题字体检查", f"三级标题应使用{h3['font']}")
        ...
        if body:
            if body.get("font"):
                ref = _add_check("body.font", body["font"], "正文字体检查", f"正文应使用{body['font']}")
                _add_fix(ref, "set_font", "body", body["font"])
            if body.get("size"):
                ref = _add_check("body.size", body["size"], "正文字号检查", f"正文应使用{body['size']}")
            ...
            if body.get("first_line_indent"):
                ref = _add_check(
                    ...
                    body["first_line_indent"],
                    "正文首行缩进检查",
                    f"正文首行应缩进{body['first_line_indent']}",
                    "P1",
                )
                _add_fix(ref, "set_first_line_indent", "body", body["first_line_indent"])
            if body.get("align"):
                ref = _add_check(
                    "body.align", body["align"], "正文对齐检查", f"正文应{self._align_label(body['align'])}", "P1"
                )
        ...
        for key, (label, margin_key) in margin_map.items():
            ...
            if val:
                ref = _add_check(
                    f"page_setup.margins.{margin_key}", val, f"{label}检查", f"{label}应为{val}（GB/T 9704标准）"
                )
        ...
        if sig and sig.get("align"):
            ref = _add_check(
                "signature.align", sig["align"], "落款对齐检查", f"落款应{self._align_label(sig['align'])}", "P1"
            )
