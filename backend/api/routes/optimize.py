...
async def convert_markdown_text(body: MarkdownConvertRequest):
    ...
    for i, p in enumerate(body.paragraphs):
        rf = RunFormat(
            font_name=p.format.get("font_name"), font_size_pt=p.format.get("font_size_pt"), bold=p.format.get("bold")
        )
        pf = ParagraphFormat(
            alignment=p.format.get("alignment"),
            first_line_indent_pt=p.format.get("first_line_indent_pt"),
            line_spacing_pt=p.format.get("line_spacing_pt"),
        )
...
def _make_simple_para(
    text: str, font_name: str, font_size_pt: int, align: str, color: str | None = None
) -> "OxmlElement":
    """创建一个简单的 OOXML 段落元素。"""
    from docx.oxml.ns import qn
    from docx.oxml import OxmlElement
...
async def markdown_to_preview(body: MarkdownToPreviewRequest):
    ...
    for i, line in enumerate(lines):
        ...
        model.paragraphs.append(
            Paragraph(
                ...
                role="body",
                is_heading=False,
                runs=[Run(index=0, text=line, format=rf)],
                format=pf,
            )
        )
    ...
    if cc_text:
        footer_paras.append(
            Paragraph(
                ...
                is_heading=False,
                runs=[
                    Run(index=0, text=f"抄送：{cc_text}", format=RunFormat(font_name="仿宋_GB2312", font_size_pt=14.0))
                ],
    ...
    printer = footer_fields.get("printer", {}).get("default", "") or header_fields.get("issuing_org", {}).get(
        "default", ""
    )
    ...
    if printer or print_date:
        ...
        footer_paras.append(
            Paragraph(
                ...
                is_heading=False,
                runs=[
                    Run(index=0, text=footer_info_text, format=RunFormat(font_name="仿宋_GB2312", font_size_pt=14.0))
                ],
    ...
    for p in model.paragraphs:
        ...
        runs_data = [
            {
                "text": r.text or "",
                "bold": getattr(r.format, "bold", None),
                "font_name": getattr(r.format, "font_name", None),
                "font_size_pt": getattr(r.format, "font_size_pt", None),
                "color": getattr(r.format, "color", None),
            }
            for r in p.runs
        ]
        result_paras.append(
            {
                ...
                "format": {
                    "alignment": p.format.alignment,
                    "first_line_indent_pt": p.format.first_line_indent_pt,
                    "left_indent_pt": getattr(p.format, "left_indent_pt", None),
                    "font_name": rf.font_name,
                    "font_size_pt": rf.font_size_pt,
                    "line_spacing_pt": p.format.line_spacing_pt,
                    "bold": rf.bold,
            ...
            cell_paras = [
                {
                    ...
                    "format": {
                        "alignment": cp.format.alignment,
                        "font_name": (cp.runs[0].format.font_name if cp.runs else None),
                        "font_size_pt": (cp.runs[0].format.font_size_pt if cp.runs else None),
                        "bold": (cp.runs[0].format.bold if cp.runs else None),
                    },
                }
                for cp in c.paragraphs
            ]
...
async def download_from_preview(body: PreviewDownloadRequest):
    ...
    if body.tables:
        for t_data in body.tables:
            ...
            table = Table(
                index=insert_idx if insert_idx >= 0 else len(model.tables),
                rows=t_data.get("rows", 0),
                cols=t_data.get("cols", 0),
                cells=[],
            ...
            for c_data in t_data.get("cells", []):
                ...
                for cp_data in c_data.get("paragraphs", []):
                    ...
                    cp_rf = RunFormat(
                        font_name=fmt.get("font_name"), font_size_pt=fmt.get("font_size_pt"), bold=fmt.get("bold")
                    )
                    ...
                    cell_paras.append(
                        Paragraph(
                            ...
                            runs=[Run(index=0, text=cp_data.get("text", ""), format=cp_rf)],
                            format=cp_pf,
                        )
                    )
                table.cells.append(
                    TableCell(row=c_data["row"], col=c_data["col"], text=c_data.get("text", ""), paragraphs=cell_paras)
                )
