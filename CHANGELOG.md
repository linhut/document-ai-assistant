...
### ✨ 新增
- AI 配置页面新增「删除配置」按钮：可彻底删除已添加的服务商配置（含加密 API Key），带二次确认；删除启用中的配置后 AI 服务自动回到未启用状态，并同步清理模型健康检测缓存

## [1.5.1] - 2026-08-06

### 🐛 修复：AI 语义优化启用失败
- 修复「配置后未启用」根因：前端以 `custom:<服务名>`（如 `custom:aliyun_qwen`）作为 DB 键保存，后端 `create_provider()` 无法按该键实例化 provider，导致 AI 语义分析 / AI 润色 / 结构分析全部静默失败。现统一在 `manager.py` 归一化 `custom:*` → `custom`，一处修复所有调用点
- 修复 AI 结构分析（parser 自动触发）此前仅匹配 `provider="openai"` 的启用配置，用户启用 DeepSeek / custom 等其他服务商时永远被跳过；改为优先取当前启用配置
- 修复「测试连接 / 获取模型」使用已保存密钥时按 `custom` 查询不到 `custom:*` 记录的问题

### 🎯 项目介绍优化
- gongwen-skill 独立发行版介绍由 README 底部「社区与友链」前置到顶部醒目位置，并新增官网首页曝光 Banner
- README 新增 gongwen-skill TOC 锚点

### ✅ 测试
- 修复 9 个遗留测试缺陷（过时断言 `title`→`doc_title`、`max_retries` 默认值、action 支持集合与实际 `_ACTION_MAP` 脱节、缺失 pytest-asyncio 的异步标记、真实 API 测试硬编码密钥）
- 全量测试：100 passed, 2 skipped（真实 API / E2E 需外部服务时自动跳过）

## [1.5.0] - 2026-07-06

### 🚀 新增
- 文档版本历史回溯，支持恢复到任意历史版本
- 批量导出功能，支持多文档一键下载
- 规则引擎模板变量支持，灵活适配单位内部编号规则

### 🔧 改进
- 优化大文档（100 页+）处理性能，内存占用降低 40%
- 更新字体映射库，新增 8 个信创系统常用字体
...
- **发文机关标志字号** — 22pt → **30pt**
- **发文机关标志颜色** — #CC0000 → **#E00000**（深红色）
- **发文机关标志字间距** — 3pt → **0**（对齐国标，无额外字距）
- **发文机关标志行高** — 新增 `line-height: 1.4`
